""" Referenced from https://github.com/Alibaba-NLP/DeepResearch/blob/main/inference/tool_visit.py """

from qwen_agent.tools.base import BaseTool, register_tool
from inference.tools.prompt import EXTRACTOR_PROMPT
...
VISIT_SERVER_TIMEOUT = int(os.environ.get("VISIT_SERVER_TIMEOUT", 200))
WEBCONTENT_MAXLENGTH = int(os.environ.get("WEBCONTENT_MAXLENGTH", 150000))
...
class Visit(BaseTool):
    ...
    def call(self, params: Union[str, dict], **kwargs) -> str:
        ...
        if isinstance(url, str):
            ...
            start_time = time.time()
            for u in tqdm(url, desc="Visiting URLs"): 
    ...
    def readpage_jina(self, url: str, goal: str) -> str:
        ...
        max_retries = int(os.environ.get('VISIT_SERVER_MAX_RETRIES', 1))
        ...
        if content and not content.startswith("[visit] Failed to read page.") and content != "[visit] Empty content." and not content.startswith("[document_parser]"):
            ...
            summary_retries = 3
            while len(raw) < 10 and summary_retries >= 0: # NOTE: if len(raw) is too short, it means the call failed, need to retry
        ...
        # If no valid content was obtained after all retries
            ...
            useful_information = "The useful information in {url} for user goal {goal} as follows: \n\n".format(url=url, goal=goal)
            # HACK: hint the model to not be too focused on a single url
            useful_information += "Evidence in page: \n" + "The provided webpage content could not be accessed. Please check the URL or choose another URL." + "\n\n"
            useful_information += "Summary: \n" + "The webpage content could not be processed, and therefore, no information is available." + "\n\n"
            return useful_information
